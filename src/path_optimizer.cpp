...
trajectory PathOptimizer::keepLane(dynamics&& dyn, const dynamics& closest_front_car, double pd_f) {
  ...
  return searchOptimizedJMT({dyn.first, dyn.second}, ps_f, vs_f, pd_f, vd_f, 1.0, 5.0);
}
...
PathOptimizer::searchOptimizedJMT(dynamics&& dyn, double ps_f, double vs_f, double pd_f, double vd_f,
  ...
  while (true) {
    ...
    delta_t += 5 * time_step_; // use a course time grid for searching
